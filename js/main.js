  ...
  streamers.forEach(function(channel) {
    $.getJSON("https://api.twitch.tv/kraken/streams/" + channel + "?callback=?", function(data) {
      ...
      $.getJSON("https://api.twitch.tv/kraken/channels/" + channel + "?callback=?", function(data) {
        ...
        commit = '<div class="fond ' + status + '"><div class="col-md-2"><img src="' + pic + '" class="picture"></div><div class="col-md-3 vert-align"><a href="' + data.url + '"  target="_blank" style="color: black; font-size: 18px">' + name + '</a></div><div class="vert-align col-md-8" style="text-align:center">' + game + '<br>' + details + '</div></div>';
        if (status == "online")
